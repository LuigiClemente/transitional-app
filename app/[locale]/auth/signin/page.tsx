...
const LOGIN_REDIRECT_PATH = '/form'; // Changed from '/documents' to '/form'
...
function PageLogin() {
  ...
  const onFormSubmit = async ({ email, password }: LoginFormInputs) => {
    // Check if the form is empty, if so redirect to /step
    if (!email && !password) {
      router.push('/step');
      return;
    }else {
  ...
  // Custom submit handler that checks for empty form
        ...
        {!showAgreement && (
          <div className="w-full bg-white rounded-lg shadow-xl md:mt-0 sm:max-w-md xl:p-0">
            <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
              ...
              <form className="space-y-4 md:space-y-6" onSubmit={customSubmit}>
                <div>
